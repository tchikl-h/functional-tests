...
appTest.get('/', (req, res) => {
  ...
  tcpPortUsed.waitForStatus(parseInt(urlparts.port), urlparts.hostname, inUse, 500, 4000)
  .then(function() {
...
function callWebhook (msg) {
  ...
  var inUse = true;   // wait until the port is in use
  console.log("Wait for port "+parseInt(urlparts.port)+" to be available...");
  tcpPortUsed.waitForStatus(parseInt(urlparts.port), urlparts.hostname, inUse, 500, 10000)
  .then(function() {
